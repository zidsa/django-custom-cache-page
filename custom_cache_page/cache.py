...
def _cache_page(
    ...
    def _cache(view_func):
        ...
        def __cache(request, *args, **kwargs):
            ...
            group_version = (
                cache.get_or_set(group, 1, timeout=versions_timeout) if versioned else 0
            )
            cache_key = hash_key(
                f"{prefix}:{group}:{group_version}:{key_func(request)}"
            )
...
def cache_page(timeout, key_func, versioned=False, group_func=None, prefix=None):
    return _cache_page(
        timeout, key_func, prefix=prefix, group_func=group_func, versioned=versioned
    )
